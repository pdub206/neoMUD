...
static void get_player_filename_local(char *filename, size_t fbufsize, const char *orig_name)
{
...

  {
    ...
    if (!build_path3(dir, sizeof(dir), PLAYER_DIR, middle, SLASH))
      return;
    strlcpy(base, name, sizeof(base));
    strlcat(base, ".", sizeof(base));
    strlcat(base, SUF_PLAYER, sizeof(base));
    if (!build_path(filename, fbufsize, dir, base))
      return;
  }
}

static void write_player_toml(FILE *out, struct char_file_u *st)
{
  int i;
  int tmp[MAX_SKILLS + 1];
  int cond[3];

  toml_write_key_string(out, "name", st->name);
  toml_write_key_string(out, "description", st->description);
  toml_write_key_string(out, "title", st->title);
  fprintf(out, "sex = %d\n", st->sex);
  fprintf(out, "class = %d\n", st->chclass);
  fprintf(out, "level = %d\n", st->level);
  fprintf(out, "hometown = %d\n", st->hometown);
  fprintf(out, "birth = %ld\n", (long)st->birth);
  fprintf(out, "played = %d\n", st->played);
  fprintf(out, "weight = %d\n", st->weight);
  fprintf(out, "height = %d\n", st->height);
  toml_write_key_string(out, "password", st->pwd);
  fprintf(out, "last_logon = %ld\n", (long)st->last_logon);
  toml_write_key_string(out, "host", st->host);

  fputs("\n[char_specials]\n", out);
  fprintf(out, "alignment = %d\n", st->char_specials_saved.alignment);
  fprintf(out, "idnum = %ld\n", st->char_specials_saved.idnum);
  fprintf(out, "act = %ld\n", st->char_specials_saved.act);
  fprintf(out, "affected_by = %ld\n", st->char_specials_saved.affected_by);
  for (i = 0; i < 5; i++)
    tmp[i] = st->char_specials_saved.apply_saving_throw[i];
  toml_write_int_array(out, "saving_throw", tmp, 5);

  fputs("\n[player_specials]\n", out);
  for (i = 0; i <= MAX_SKILLS; i++)
    tmp[i] = st->player_specials_saved.skills[i];
  toml_write_int_array(out, "skills", tmp, MAX_SKILLS + 1);
  fprintf(out, "padding0 = %d\n", st->player_specials_saved.PADDING0);
  fprintf(out, "talks = [");
  for (i = 0; i < MAX_TONGUE; i++) {
    if (i)
      fputs(", ", out);
    fputs(st->player_specials_saved.talks[i] ? "true" : "false", out);
  }
  fputs("]\n", out);
  fprintf(out, "wimp_level = %d\n", st->player_specials_saved.wimp_level);
  fprintf(out, "freeze_level = %d\n", st->player_specials_saved.freeze_level);
  fprintf(out, "invis_level = %d\n", st->player_specials_saved.invis_level);
  fprintf(out, "load_room = %d\n", st->player_specials_saved.load_room);
  fprintf(out, "pref = %ld\n", st->player_specials_saved.pref);
  fprintf(out, "bad_pws = %d\n", st->player_specials_saved.bad_pws);
  ...
  toml_write_int_array(out, "conditions", cond, 3);
  ...
  fputs("\n[points]\n", out);
  fprintf(out, "mana = %d\n", st->points.mana);
  fprintf(out, "max_mana = %d\n", st->points.max_mana);
  fprintf(out, "hit = %d\n", st->points.hit);
  fprintf(out, "max_hit = %d\n", st->points.max_hit);
  fprintf(out, "move = %d\n", st->points.move);
  fprintf(out, "max_move = %d\n", st->points.max_move);
  fprintf(out, "armor = %d\n", st->points.armor);
  fprintf(out, "gold = %d\n", st->points.gold);
  fprintf(out, "bank_gold = %d\n", st->points.bank_gold);
  fprintf(out, "exp = %d\n", st->points.exp);
  fprintf(out, "hitroll = %d\n", st->points.hitroll);
  fprintf(out, "damroll = %d\n", st->points.damroll);
  ...
  for (i = 0; i < MAX_AFFECT; i++) {
    ...
    fprintf(out, "bitvector = %ld\n", st->affected[i].bitvector);
  }
}

static void convert_players(void)
{
  FILE *fl;
  struct char_file_u player;

  if (!(fl = fopen(PLAYER_FILE, "rb"))) {
    fprintf(stderr, "No playerfile %s found; skipping.\n", PLAYER_FILE);
    return;
  }
  ...
  while (!feof(fl)) {
    ...
    if (fread(&player, sizeof(player), 1, fl) != 1)
      break;
    if (!*player.name)
      continue;

    ensure_player_dirs(player.name);
    get_player_filename_local(filename, sizeof(filename), player.name);

    if (!(out = fopen(filename, "w")))
      continue;
    write_player_toml(out, &player);
    fclose(out);
  }
